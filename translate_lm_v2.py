...
tf.app.flags.DEFINE_string("train_dir", os.path.normpath("./t2_t3_merge_experiments_v2/"), "Training directory.")
...
tf.app.flags.DEFINE_integer("save_every", 5000, "How often to save model.")
...
def create_model(session, actions, sampling=False):
  ...
  model = motion_rnn_lm_v2.MotionRNNModelLM(
      FLAGS.architecture,
      FLAGS.loop_type,
...
def train():
  ...
  with tf.Session(config=tf.ConfigProto( gpu_options=gpu_options, device_count = device_count )) as sess:
    ...
    for _ in xrange( FLAGS.iterations ):
      ...
      # === Training step ===
      encoder_inputs, encoder_outputs, plan_inputs, plan_init_state, decoder_inputs, decoder_outputs = model.get_batch( train_set, FLAGS.omit_one_hot ) # modified
      is_training = True
      use_sample = False
      dropout_prob = 0.3
      ...
      _, step_loss, step_sampling_loss, loss_summary, lr_summary = model.step( sess, encoder_inputs, encoder_outputs, plan_inputs, plan_init_state, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, False)
      ...
      if current_step % FLAGS.test_every == 0:
        ...
        encoder_inputs, encoder_outputs, plan_inputs, plan_init_state, decoder_inputs, decoder_outputs = model.get_batch( test_set, FLAGS.omit_one_hot ) # modified
        step_loss, loss_summary = model.step(sess, encoder_inputs, encoder_outputs, plan_inputs, plan_init_state, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, forward_only )
        ...
        for action in actions:
          ...
          # Pretty print of the results for 80, 160, 320, 400, 560, 1000, 2000 and 4000 ms
          print("{0: <16} |".format(action), end="")
          for ms in [1,3,7,9,13,24]:
            if FLAGS.seq_length_out >= ms+1:
              print(" {0:.3f} +- {1:.5f} | ".format( mean_mean_errors[ms] , n_trial_std_errors[ms]), end="")
            else:
              print("   n/a |", end="")
          print()
...
def sample():
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count )) as sess:
    ...
    for action in actions:
      ...
      srnn_loss, srnn_poses, _ = model.step(sess, encoder_inputs, encoder_outputs, plan_inputs, plan_init_state, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, forward_only, srnn_seeds)
