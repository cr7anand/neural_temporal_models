...
class MotionRNNModelSimpleLM(object):
  ...
  def __init__(self,
    ...
    with tf.name_scope("inputs"):
      ...
      enc_in = tf.placeholder(dtype, shape=[None, source_seq_len, self.HUMAN_SIZE], name="enc_in")
      enc_out = tf.placeholder(dtype, shape=[None, source_seq_len, self.HUMAN_SIZE], name="enc_out") 
      dec_in = tf.placeholder(dtype, shape=[None, target_seq_len, self.HUMAN_SIZE], name="dec_in")
      ...
      enc_in = tf.reshape(enc_in, [-1, self.HUMAN_SIZE])
      enc_out = tf.reshape(enc_out, [-1, self.HUMAN_SIZE])
      dec_in = tf.reshape(dec_in, [-1, self.HUMAN_SIZE])
	  ...
	  with tf.name_scope("body_rnn_future"):
		...
		for i in range(int(target_seq_len/cond_len)):
				if i % 2 == 0: # open loop
					...
					future_output_j = future_outputs[-1]
  ...
  def get_batch_srnn(self, data, action ):
    ...
    for i in xrange( batch_size ):
      ...
      encoder_inputs[i, :, :]  = data_sel[0:source_seq_len, 0:self.HUMAN_SIZE]
      encoder_outputs[i, :, :] = data_sel[1:source_seq_len+1, 0:self.HUMAN_SIZE]
      
      decoder_inputs[i, :, :]  = data_sel[source_seq_len:(source_seq_len+target_seq_len), 0:self.HUMAN_SIZE]
