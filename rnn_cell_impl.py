...
class BasicRNNCell(RNNCell):
  ...
  def __init__(self, num_units, activation=None, reuse=None, apply_layer_norm=True): # modified layer_norm
    ...
    if self._apply_layer_norm:
      self._layer_norm = tf.contrib.layers.layer_norm # modified layer_norm
  ...
  def call(self, inputs, state):
    ...
    if self._apply_layer_norm:
      output = self._activation(self._layer_norm(self._linear([inputs, state]))) # modified layer_norm
...
class GRUCell(RNNCell):
  ...
  def call(self, inputs, state):
    ...
    if self._apply_layer_norm:
      value = math_ops.sigmoid(self._layer_norm(self._gate_linear([inputs, state]))) # modified layer_norm
    ...
    if self._apply_layer_norm:
      c = self._activation(self._layer_norm(self._candidate_linear([inputs, r_state]))) # modified layer_norm
    else:   
