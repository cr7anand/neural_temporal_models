...
import motion_rnn_lm

# Learning
tf.app.flags.DEFINE_float("learning_rate", .0002, "Learning rate.")
tf.app.flags.DEFINE_float("learning_rate_decay_factor", 0.8, "Learning rate is multiplied by this much. 1 means no decay.")
...
tf.app.flags.DEFINE_string("loop_type", "closed", "loop type to use: [open, closed].")
tf.app.flags.DEFINE_integer("body_size", 128, "Size of each body rnn model layer.")
tf.app.flags.DEFINE_string("body_cell", "gru", "RNN cell type of body rnn : [elman, lstm, gru]")
tf.app.flags.DEFINE_integer("plan_size", 128, "Size of each plan rnn model layer.")
tf.app.flags.DEFINE_string("plan_cell", "gru", "RNN cell type of body rnn : [elman, lstm, gru]")
...
def create_model(session, actions, sampling=False):
  ...
  model = motion_rnn_lm.MotionRNNModelLM(
      FLAGS.architecture,
      FLAGS.loop_type,
...
def train():
  ...
  with tf.Session(config=tf.ConfigProto( gpu_options=gpu_options, device_count = device_count )) as sess:
    ...
    for _ in xrange( FLAGS.iterations ):
      ...
      # === Training step ===
      encoder_inputs, encoder_outputs, plan_inputs, decoder_inputs, decoder_outputs = model.get_batch( train_set, FLAGS.omit_one_hot ) # modified
      _, step_loss, loss_summary, lr_summary = model.step( sess, encoder_inputs, encoder_outputs, plan_inputs, decoder_inputs, decoder_outputs, False )
      ...
      if current_step % FLAGS.test_every == 0:
        ...
        encoder_inputs, encoder_outputs, plan_inputs, decoder_inputs, decoder_outputs = model.get_batch( test_set, FLAGS.omit_one_hot ) # modified
        step_loss, loss_summary = model.step(sess, encoder_inputs, encoder_outputs, plan_inputs, decoder_inputs, decoder_outputs, forward_only)
        ...
        for action in actions:
          ...
          encoder_inputs, encoder_outputs, plan_inputs, decoder_inputs, decoder_outputs = model.get_batch_srnn( test_set, action )
          srnn_loss, srnn_poses, _ = model.step(sess, encoder_inputs, encoder_outputs, plan_inputs, decoder_inputs, decoder_outputs, True, True)
...
def get_srnn_gts( actions, model, test_set, data_mean, data_std, dim_to_ignore, one_hot, to_euler=True ):
  ...
  for action in actions:
    ...
    srnn_gt_euler = []
    _,_, _, _, srnn_expmap = model.get_batch_srnn( test_set, action ) # modified
...
def sample():
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count )) as sess:
    ...
    for action in actions:
      ...
      forward_only = True
      srnn_seeds = True
      srnn_loss, srnn_poses, _ = model.step(sess, encoder_inputs, encoder_outputs, plan_inputs, decoder_inputs, decoder_outputs, forward_only, srnn_seeds)
