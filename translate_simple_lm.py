...
tf.app.flags.DEFINE_integer("learning_rate_step", 1500, "Every this many steps, do decay.")
...
tf.app.flags.DEFINE_string("body_cell", "gru", "RNN cell type of body rnn : [elman, lstm, gru]")
...
tf.app.flags.DEFINE_string("train_dir", os.path.normpath("./final_exp_samples/simple_gru_no_plan_no_deriv"), "Training directory.")
...
def create_model(session, actions, sampling=False):
  ...
  model = motion_rnn_simple_lm.MotionRNNModelSimpleLM(
      FLAGS.architecture,
      FLAGS.loop_type,
...
def train():
  ...
  with tf.Session(config=tf.ConfigProto( gpu_options=gpu_options, device_count = device_count )) as sess:
    ...
    for _ in xrange( FLAGS.iterations ):
      ...
      # === Training step ===
      encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs = model.get_batch( train_set, FLAGS.omit_one_hot ) # modified
      ...
      _, step_loss, step_sampling_loss, loss_summary, lr_summary = model.step( sess, encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, False )      

      model.train_writer.add_summary( loss_summary, current_step )
      model.train_writer.add_summary( lr_summary, current_step )

      if current_step % 10 == 0:
        print("step {0:04d}; step_loss: {1:.4f}; sampling_loss: {2:.4f}".format(current_step, step_loss, sampling_weight*step_sampling_loss ))
      ...
      # === step decay ===
      if current_step % FLAGS.learning_rate_step == 0: #and current_step <= 60000:
      ...
      if current_step % FLAGS.test_every == 0:
        ...
        encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs = model.get_batch( test_set, FLAGS.omit_one_hot ) # modified
        step_loss, loss_summary = model.step(sess, encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, forward_only)
        ...
        for action in actions:
          ...
          encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs = model.get_batch_srnn( test_set, action )

          is_training = False 
          use_sample = True
          dropout_prob = 0.0

          srnn_loss, srnn_poses, _ = model.step(sess, encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, True, True)
...
def get_srnn_gts( actions, model, test_set, data_mean, data_std, dim_to_ignore, one_hot, to_euler=True ):
  ...
  for action in actions:
    ...
    srnn_gt_euler = []
    _, _, _, srnn_expmap = model.get_batch_srnn( test_set, action ) # modified

    # expmap -> rotmat -> euler
    for i in np.arange( srnn_expmap.shape[0] ):
      denormed = data_utils.unNormalizeData(srnn_expmap[i,:,:], data_mean, data_std, dim_to_ignore, actions, one_hot ) # modified
...
def sample():
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count )) as sess:
    ...
    # === Create the model ===
    print("Creating %d layers of %d units." % (FLAGS.num_layers, FLAGS.body_size))
    sampling     = True
    ...
    # === Read and denormalize the gt with srnn's seeds, as we'll need them
    # many times for evaluation in Euler Angles ===
    srnn_gts_expmap = get_srnn_gts( actions, model, test_set, data_mean, data_std, dim_to_ignore, FLAGS.omit_one_hot, to_euler=False )
    srnn_gts_euler = get_srnn_gts( actions, model, test_set, data_mean, data_std, dim_to_ignore, FLAGS.omit_one_hot )
    ...
    for action in actions:
      ...
      srnn_loss, srnn_poses, _ = model.step(sess, encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, forward_only, srnn_seeds)
