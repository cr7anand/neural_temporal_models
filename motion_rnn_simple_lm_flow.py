...
class MotionRNNModelSimpleLM(object):
  ...
  def __init__(self,
	...
	body_rnn_cell = rnn_cell_impl.BasicRNNCell( self.body_rnn_size, kernel_initializer=k_init, bias_initializer=b_init) 	

    elif self.body_cell_type == "elman" and num_layers > 1:
	body_rnn_cell = rnn_cell_impl.MultiRNNCell( [rnn_cell_impl.BasicRNNCell(self.body_rnn_size, kernel_initializer=k_init, bias_initializer=b_init) for _ in range(num_layers)] ) 

    elif self.body_cell_type == "lstm" and num_layers==1:
	body_rnn_cell = rnn_cell_impl.BasicLSTMCell( self.body_rnn_size, kernel_initializer=k_init, bias_initializer=b_init)

    elif self.body_cell_type == "lstm" and num_layers > 1:   
	body_rnn_cell = rnn_cell_impl.MultiRNNCell( [rnn_cell_impl.BasicLSTMCell(self.body_rnn_size, kernel_initializer=k_init, bias_initializer=b_init) for _ in range(num_layers)] )

    elif self.body_cell_type == "gru" and num_layers==1:
	body_rnn_cell = rnn_cell_impl.GRUCell( self.body_rnn_size, kernel_initializer=k_init, bias_initializer=b_init)

    elif self.body_cell_type == "gru" and num_layers > 1:
	body_rnn_cell = rnn_cell_impl.MultiRNNCell( [rnn_cell_impl.GRUCell(self.body_rnn_size, kernel_initializer=k_init, bias_initializer=b_init) for _ in range(num_layers)] )	

    elif self.body_cell_type == "delta" and num_layers==1:
        body_rnn_cell = deltaRNN.DeltaRNNCell( self.body_rnn_size )

    elif self.body_cell_type == "delta" and num_layers > 1:
	body_rnn_cell = rnn_cell_impl.MultiRNNCell( [deltaRNN.DeltaRNNCell(self.body_rnn_size ) for _ in range(num_layers)] )
    ...
    body_rnn_cell = rnn_cell_impl.DropoutWrapper(body_rnn_cell, input_keep_prob = 1 - self.dropout_prob, output_keep_prob = 1 - self.dropout_prob) 

    # Add Linear output decoder
    body_rnn_cell = rnn_cell_extensions.LinearSpaceDecoderWrapper( body_rnn_cell, self.HUMAN_SIZE )

    # Finally, wrap everything in a residual layer if we want to model velocities
    if residual_velocities:
      body_rnn_cell = rnn_cell_extensions.ResidualWrapperv2( body_rnn_cell, self.HUMAN_SIZE )

    # Body-RNN
    with tf.name_scope("body_rnn_past"):
      # Run Body-RNN for past frames (use gt-input at each timestep)
      past_pred_outputs, past_state = rnn.static_rnn(body_rnn_cell, enc_in, dtype=tf.float32) 
    ...
    def future_test_graph(target_seq_len, past_state, past_pred_outputs, body_rnn_cell, enc_out, dec_out ):
		...
		for i in range(target_seq_len): # last future frame input ignored as gt not available
			future_output_i, future_state = body_rnn_cell(future_output_i, future_state) # using cell state at end of past frames 
			...
			p_prev_time_output = prev_time_output
                ...
                with tf.name_scope("loss_angles"):
      			past_loss_angles = tf.square(tf.subtract(enc_out, past_pred_outputs))
      			future_loss_angles = tf.square(tf.subtract(dec_out, future_pred_outputs))

			norm_l2 = tf.reduce_mean(tf.concat([past_loss_angles, future_loss_angles], 0))
      			loss_angles = norm_l2 # test time loss only has mse_loss no sampling loss 
    ...
    def future_train_graph(loop_type, past_state, past_pred_outputs, target_seq_len, body_rnn_cell):
	  ...
	  with tf.name_scope("body_rnn_future"):
		...
		future_pred_outputs, future_state = rnn.static_rnn(body_rnn_cell, dec_in, initial_state=past_state, dtype=tf.float32) 
	  ...
	  with tf.name_scope("body_rnn_future"):
		...
		for i in range(int(target_seq_len/cond_len)):
				if i % 2 == 0: # open loop
					future_outputs, future_state = rnn.static_rnn(body_rnn_cell, dec_in[i*cond_len:(i+1)*cond_len], initial_state=future_state, dtype=tf.float32)
					...
					for j in range(cond_len):
						future_output_j, future_state = body_rnn_cell(future_output_j, future_state)
    ...
    def sampler_graph(use_sample, past_state, past_pred_outputs, target_seq_len, body_rnn_cell, enc_out, dec_out):
		...
		norm_l2 = tf.reduce_mean(tf.concat([past_loss_angles, future_loss_angles], 0))
        	loss_angles = norm_l2 + self.sampling_weight*sampling_loss  # train time loss has both mse_loss and sampling loss  
  ...
  def step(self, session, encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, forward_only, srnn_seeds=False ):
  ...
  def get_batch( self, data, actions ):
    ...
    encoder_inputs  = np.zeros((self.batch_size, self.source_seq_len, 4*self.HUMAN_SIZE), dtype=float)
    encoder_outputs = np.zeros((self.batch_size, self.source_seq_len, self.HUMAN_SIZE), dtype=float) 
    decoder_inputs  = np.zeros((self.batch_size, self.target_seq_len, 4*self.HUMAN_SIZE), dtype=float)
    ...
    for i in xrange( self.batch_size ):
      ...
      decoder_outputs[i,:,0:self.HUMAN_SIZE] = data_sel[self.source_seq_len+1:total_frames+1, 0:self.HUMAN_SIZE]
  ...
  def get_batch_srnn(self, data, action ):
    ...
    encoder_inputs  = np.zeros( (batch_size, source_seq_len, 4*self.HUMAN_SIZE), dtype=float )
    encoder_outputs = np.zeros((batch_size, source_seq_len, self.HUMAN_SIZE), dtype=float)
   
    decoder_inputs  = np.zeros( (batch_size, target_seq_len, 4*self.HUMAN_SIZE), dtype=float )
    ...
    for i in xrange( batch_size ):
      ...
      encoder_outputs[i, :, 0:self.HUMAN_SIZE] = data_sel[1:source_seq_len+1, 0:self.HUMAN_SIZE]
      ...
      decoder_outputs[i, :, 0:self.HUMAN_SIZE] = data_sel[source_seq_len+1:(source_seq_len+target_seq_len+1), 0:self.HUMAN_SIZE]
