...
import motion_rnn_simple_lm_flow
...
tf.app.flags.DEFINE_string("train_dir", os.path.normpath("./simple_lm_flow_experiments/"), "Training directory.")
...
def create_model(session, actions, sampling=False):
  ...
  model = motion_rnn_simple_lm_flow.MotionRNNModelSimpleLM(
      FLAGS.architecture,
      FLAGS.loop_type,
...
def train():
  ...
  with tf.Session(config=tf.ConfigProto( gpu_options=gpu_options, device_count = device_count )) as sess:
    ...
    for _ in xrange( FLAGS.iterations ):
      ...
      _, step_loss, step_sample_loss, loss_summary, lr_summary = model.step( sess, encoder_inputs, encoder_outputs, decoder_inputs, decoder_outputs, is_training, use_sample, dropout_prob, sampling_weight, False)
      ...
      if current_step % 10 == 0:
        print("step {0:04d}; step_loss: {1:.4f}; step_sample_loss: {2:.4f}".format(current_step, step_loss, sampling_weight*step_sample_loss ))
...
def sample():
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count )) as sess:
    ...
    for action in actions:
      ...
      # denormalizes too
      srnn_pred_expmap = data_utils.revert_output_format( srnn_poses, data_mean, data_std, dim_to_ignore, actions, FLAGS.omit_one_hot )
