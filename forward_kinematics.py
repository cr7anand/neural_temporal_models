...
def main():
  ...
  with h5py.File( 'samples.h5', 'r' ) as h5f:
    expmap_pred = h5f['expmap/preds/' + action + '_' + test_set_sequence][:]
  ...
  metadata = dict(title= action + '_' + test_set_sequence, artist='Matplotlib', comment='Movie support!')
  writer = FFMpegWriter(fps=25, codec="libx264", bitrate=-1, metadata=metadata)
