...
class MotionRNNModelLM(object):
  ...
  def __init__(self,
    ...
    self.iterations = 0 # keeps track of train iters for noise scheduling
    ...
    if self.plan_cell_type == "elman" and self.num_layers==1:    
	plan_rnn_cell = rnn_cell_impl.BasicRNNCell( self.plan_rnn_size , kernel_initializer=k_init, bias_initializer=b_init)	
    
    elif self.plan_cell_type == "elman" and self.num_layers > 1:
        plan_rnn_cell = rnn_cell_impl.MultiRNNCell( [rnn_cell_impl.BasicRNNCell(self.plan_rnn_size, kernel_initializer=k_init, bias_initializer=b_init) for _ in range(num_layers)] ) 

    elif self.plan_cell_type == "lstm" and self.num_layers==1:
	plan_rnn_cell = rnn_cell_impl.BasicLSTMCell( self.plan_rnn_size , kernel_initializer=k_init, bias_initializer=b_init)    

    elif self.plan_cell_type == "lstm" and self.num_layers > 1:
        plan_rnn_cell = rnn_cell_impl.MultiRNNCell( [rnn_cell_impl.BasicLSTMCell(self.plan_rnn_size, kernel_initializer=k_init, bias_initializer=b_init) for _ in range(num_layers)] )

    elif self.plan_cell_type == "gru" and self.num_layers==1:
    ...
    elif self.plan_cell_type == "delta" and self.num_layers==1:
        plan_rnn_cell = deltaRNN.DeltaRNNCell( self.plan_rnn_size, apply_layer_norm=False )

    elif self.plan_cell_type == "delta" and self.num_layers > 1:
        plan_rnn_cell = rnn_cell_impl.MultiRNNCell( [deltaRNN.DeltaRNNCell(self.plan_rnn_size, apply_layer_norm=False) for _ in range(num_layers)])	 	

    # === Create Body RNN (Forward-RNN) ===
    if self.body_cell_type == "elman" and self.num_layers==1:    
    ...
    elif self.body_cell_type == "elman" and self.num_layers > 1: 
        body_rnn_cell = rnn_cell_implement.MultiRNNCell( [rnn_cell_implement.BasicRNNCell(self.body_rnn_size ) for _ in range(num_layers)] )    

    elif self.body_cell_type == "lstm" and self.num_layers==1:
	body_rnn_cell = rnn_cell_implement.BasicLSTMCell( self.body_rnn_size , kernel_initializer=k_init, bias_initializer=b_init) # using modified LSTM cell def to inlcude plan RNN
        ...
        body_rnn_cell = rnn_cell_implement.MultiRNNCell( [rnn_cell_implement.BasicLSTMCell(self.body_rnn_size, kernel_initializer=k_init, bias_initializer=b_init) for _ in range(num_layers)] ) 
    
    elif self.body_cell_type == "gru" and self.num_layers==1:
    ...
    elif self.body_cell_type == "delta" and self.num_layers==1:
    ...
    #plan_rnn_cell = rnn_cell_impl.DropoutWrapper(plan_rnn_cell, input_keep_prob = 1 - self.dropout_prob, output_keep_prob = 1 - self.dropout_prob, variational_recurrent=True, input_size=self.n_actions,dtype=tf.float32)
    ...
    if residual_velocities:
      body_rnn_cell = body_rnn_cell_extensions.ResidualWrapper( body_rnn_cell )

    # Re-parameterization trick to learn random init state of plan RNN
  ...
  def get_batch( self, data, actions ):
    ...
    for i in xrange( self.batch_size ):
      ...
      # Add the data
      encoder_inputs[i,:,:]  = data_sel[0:self.source_seq_len, 0:self.HUMAN_SIZE]
      ...
      decoder_inputs[i,:,:]  = data_sel[self.source_seq_len:total_frames, 0:self.HUMAN_SIZE]
