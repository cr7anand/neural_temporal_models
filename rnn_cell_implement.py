  ...
  def __call__(self, inputs, state, context, scope=None): # modified
    ...
    if scope is not None:
      with vs.variable_scope(scope, custom_getter=self._rnn_get_variable) as scope:
        return super(RNNCell, self).__call__(inputs, state, context, scope=scope) # modified
    else:
      with vs.variable_scope(vs.get_variable_scope(), custom_getter=self._rnn_get_variable):
        return super(RNNCell, self).__call__(inputs, state, context) # modified 
...
class BasicRNNCell(RNNCell):
  ...
  def __init__(self, num_units, activation=None, reuse=None, apply_layer_norm=False):
  ...
  def call(self, inputs, state, context): # changed the call to include context
    """Most basic RNN: output = new_state = act(W * input + U * state + B)."""
     
    """ Modified basic RNN output = new_state = act(W * input + U * state + V * context + B) """
    if self._linear is None:
      self._linear = _Linear([inputs, state, context], self._num_units, True) # modified

    if self._apply_layer_norm:
      output = self._activation(self._layer_norm(self._linear([inputs, state, context]))) # modified layer_norm
    else:
      output = self._activation(self._linear([inputs, state, context])) # modified
...
class GRUCell(RNNCell):
  ...
  def call(self, inputs, state, context): # modified: changed the call to include context
    ...
    if self._gate_linear is None:
      ...
      with vs.variable_scope("gates"):  # Reset gate and update gate.
        self._gate_linear = _Linear(
            [inputs, state, context], # modified
            3 * self._num_units,      # modified  
            True,
            bias_initializer=bias_ones,
            kernel_initializer=self._kernel_initializer)

    if self._apply_layer_norm:
      value = math_ops.sigmoid(self._layer_norm(self._gate_linear([inputs, state, context]))) # modified layer_norm

    else:
      value = math_ops.sigmoid(self._gate_linear([inputs, state, context])) # modified
 
    r, u, ctxt = array_ops.split(value=value, num_or_size_splits=3, axis=1) # modified

    r_state = r * state
    if self._candidate_linear is None:
      with vs.variable_scope("candidate"):
        self._candidate_linear = _Linear(
            [inputs, r_state, ctxt], 		# modified
            self._num_units,
            True,
    ...
    if self._apply_layer_norm:
      c = self._activation(self._layer_norm(self._candidate_linear([inputs, r_state, ctxt])))   # modified layer_norm 
    else: 
      c = self._activation(self._candidate_linear([inputs, r_state, ctxt]))   # modified
    new_h = u * state + (1 - u) * c
    return new_h, new_h
...
class BasicLSTMCell(RNNCell):
  ...
  def call(self, inputs, state, context): # modified
    ...
    if self._linear is None:
      self._linear = _Linear([inputs, h, context], 4 * self._num_units, True) # modified
    # i = input_gate, j = new_input, f = forget_gate, o = output_gate
    i, j, f, o = array_ops.split(
        value=self._linear([inputs, h, context]), num_or_size_splits=4, axis=1) # modified
    ...
    if not self._variational_recurrent:
      def dropout(i, do_dropout, v):
        if not isinstance(do_dropout, bool) or do_dropout:
          return nn_ops.dropout(v, keep_prob=keep_prob, seed=self._gen_seed(salt_prefix, i))
        else:
          return v
  ...
  def call(self, inputs, state, context): # modified
    ...
    for i, cell in enumerate(self._cells):
      with vs.variable_scope("cell_%d" % i):
        ...
        cur_inp, new_state = cell(cur_inp, cur_state, cur_ctxt) # modified
